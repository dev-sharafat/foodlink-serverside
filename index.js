...
app.use(
  cors({
    origin: ["http://localhost:5173", "https://rad-hamster-5d56ec.netlify.app"],
...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.xicmgsk.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const charityRequestCollection = client
      .db("food_link_DB")
      .collection("cherity-requests");
    ...
    // restaurant
    app.get(
      "/donation-requests-restaurant/:email",
      verifyToken,
      async (req, res) => {
        const email = req.params.email;
    ...
    app.patch("/donation-request-accept/:id", verifyToken, async (req, res) => {
      try {
        const id = req.params.id;
        const targetRequest = await donationRequestCollection.findOne({
          _id: new ObjectId(id),
        });
        if (!targetRequest) {
          return res
    ...
    app.post("/cherity-role-request", verifyToken, async (req, res) => {
      ...
      const result = await charityRequestCollection.insertOne(
        paymentInfoCherity
      );
      res.send(result);
    });
    ...
    app.get("/charity-request-status/:id", verifyToken, async (req, res) => {
      ...
      const result = await charityRequestCollection.findOne(filter, {
        sort: { createdAt: -1 },
        projection: { status: 1 },
      });
    ...
    app.patch("/cherity-role-request/:id", verifyToken, async (req, res) => {
      ...
      if (action === "Approved") {
        ...
        return res
          .status(400)
          .send({ message: "Invalid action (use Approved or Rejected)" });
      }
    ...
    app.get("/donation-stats/:email", verifyToken, async (req, res) => {
      try {
        const email = req.params.email;
